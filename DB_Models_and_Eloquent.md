## 数据库模型与 Eloquent (71 提示)
  ...
  - [Eloquent where日期方法](#eloquent-where日期方法)
  - [增量和减量](#增量和减量)
  ...
  - [复制一个模型](#复制一个模型)
  - [降低内存占用](#降低内存占用)
  - [忽略 $fillable/$guarded 并强制执行](#忽略-fillableguarded-并强制执行)
  - [3层父子级结构](#3层父子级结构)
  ...
  - [在 Laravel 中使用doesntExist()方法](#在-laravel-中使用doesntexist方法)
  - [在一些模型的 boot () 方法中自动调用一个特性](#在一些模型的-boot--方法中自动调用一个特性)
  - [Laravel 的 find () 方法，比只传一个 ID 更多的选择](#laravel-的-find--方法比只传一个-id-更多的选择)
  ...
  - [Query构造器的crossJoinSub方法](#query构造器的crossjoinsub方法)
  - [belongsToMany的中间表命名](#belongstomany的中间表命名)
  - [根据Pivot字段排序](#根据pivot字段排序)
  - [从数据库中查询一条记录](#从数据库中查询一条记录)
  - [记录自动分块](#记录自动分块)
  - [定时清理过期记录中的模型](#定时清理过期记录中的模型)
  - [不变的日期和对它们的强制转换](#不变的日期和对它们的强制转换)
  - [findorfail方法也接收ids数组](#findorfail方法也接收ids数组)
  - [从你的数据库中自动移除模型prunableTrait](#从你的数据库中自动移除模型prunabletrait)
  - [日期转换](#日期转换)
  - [多模型更新插入](#多模型更新插入)
  - [过滤结果集之后获取查询构造器](#过滤结果集之后获取查询构造器)
  - [选择聚合计算相关模型](#选择聚合计算相关模型)
  - [自定义强制转换](#自定义强制转换)
  - [保存中不要触发事件](#保存中不要触发事件)
  - [基于相关模型的平均值或总数排序](#基于相关模型的平均值或总数排序)
  - [返回事务结果](#返回事务结果)
  - [从query中移除多个公共scope](#从query中移除多个公共scope)
  - [JSON列属性排序](#json列属性排序)
  - [从第一个结果中获取单列的值](#从第一个结果中获取单列的值)
  - [检测模型属性是否被修改](#检测模型属性是否被修改)
  - [定义访问器与修改器的新方法](#定义访问器与修改器的新方法)
  - [另外一种定义访问器与修改器的方法](#另外一种定义访问器与修改器的方法)
